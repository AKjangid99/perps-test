...
import type { EngineCommandType, EngineRequest, EngineResponse } from "../types/engineRequest.js";
import { resolveEngineResponse, waitForEngineResponse } from "../store/pendingResponse.js";



const publisher = createClient({ url: env.redisUrl }).on("error", (error) => console.error("Redis publisher error", error))
const subscriber = createClient({ url: env.redisUrl }).on("error", (error) => console.error("Redis subscriber error", error))
...
export async function sendToEngine(type: EngineCommandType, payload: Record<string, unknown>): Promise<EngineResponse> {
    ...
    await publisher.lPush(env.responseQueue, JSON.stringify(message))
